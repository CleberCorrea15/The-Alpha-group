...
# =====================================
def simulate_single_run(theta, steps=102000):
...
def plot_single_trajectory(trajectory, theta, dims=(0,1,2)):
    ...
    ax.scatter(x[0], y[0], z[0], color='lime', label='Start', s=50, edgecolors='black')
    ax.scatter(x[-1], y[-1], z[-1], color='red', label='End', s=50, edgecolors='black')

    ax.set_xlabel(f'Dimension {dims[0]+1}')
    ax.set_ylabel(f'Dimension {dims[1]+1}')
    ax.set_zlabel(f'Dimension {dims[2]+1}')
    ax.set_title(f'Single Trajectory under $M(\\theta)$\n$\\theta = {theta:.3f}$, {len(x):,} steps', pad=20)
...
if __name__ == "__main__":
    ...
    #theta = 0.001
    #theta = np.pi/4
    steps = 120000   # Número aumentado de passos para trajetória única
    ...
    # Se sua figura for um conjunto de pontos 3D:
cubical = CubicalComplex(top_dimensional_cells=trajectory)
cubical.compute_persistence() # Add this line
betti = cubical.persistent_betti_numbers(from_value=0, to_value=1)
print(f"Buracos dimensionais (Betti numbers): {betti}")

# Convert the trajectory data into a binary representation
# This is a simplified example and may need adjustment based on your data and needs
# One approach is to discretize the space and mark cells that contain trajectory points

# Determine the bounds of your data
min_coords = trajectory.min(axis=0)
max_coords = trajectory.max(axis=0)

# Define the resolution of your grid (e.g., number of cells per dimension)
resolution = 50  # Adjust as needed

# Create a grid to discretize the space
grid_shape = tuple([resolution] * trajectory.shape[1])
binary_grid = np.zeros(grid_shape, dtype=bool)

# Map trajectory points to grid cells
# This is a simple mapping; you might need a more sophisticated approach
cell_indices = ((trajectory - min_coords) / (max_coords - min_coords) * (resolution - 1)).astype(int)
cell_indices = np.clip(cell_indices, 0, resolution - 1) # Clip to handle potential out-of-bounds

# Mark cells that contain trajectory points
for idx in cell_indices:
    binary_grid[tuple(idx)] = True

# Now you can use binary_grid with CubicalComplex
# cubical = CubicalComplex(top_dimensional_cells=binary_grid)
# cubical.compute_persistence()
# betti = cubical.persistent_betti_numbers(from_value=0, to_value=1)
# print(f"Buracos dimensionais (Betti numbers): {betti}")

print("Created a binary grid representation of the trajectory.")
print(f"Binary grid shape: {binary_grid.shape}")
